...
class LoraKGE_Layers(BaseModel):
    ...
    def get_combined_entity_embeddings(self):
        """Return full composed entity embeddings (base + LoRA + reordered)."""
        ent_embeddings, rel_embeddings = self.embedding('Eval')  # base frozen layers
        
        if self.args.snapshot == 0:
            return ent_embeddings.detach().cpu().numpy(), rel_embeddings.detach().cpu().numpy()
        ...
        # Apply the same entity reordering as in predict()
        ...
        return all_ent_embeddings.detach().cpu().numpy(), all_rel_embeddings.detach().cpu().numpy()
