...
class experiment():
    ...
    def train(self):
        ...
        if int(self.args.snapshot_num) > self.args.snapshot + 1:  # not the last snapshot
    ...
    def continual_learning(self):
        ...
        for ss_id in range(int(self.args.snapshot_num)):
            ...
            # Here they expand the embeddings if needed, and they test again to see if it was a good
            # expansion.
            if ss_id < int(self.args.snapshot_num) - 1:
        ...
        exp_id = args.dataset  + '-SAGE-' + str(args.init) + '-' + str(args.learning_rate) + '-' + str(args.RN) + '-' + str(args.epoch_num) + '-' + str(args.seed) + str(time.time()).split('.')[0] + '.json'
